...
| 14 | 2026-08-07 | Section-minted generations (ADR 0017 §4): nameplate articles' per-generation sections mint, all-or-nothing per article, the body veto in placement, labeled-subrange parse amendment. |
| 15 | 2026-08-07 | Generation inventory hygiene (ADR 0018): the `NOT_A_GENERATION` gate in both passes, the `section-main:` target fetch with its grain guards, dash-template normalization. |
| 16 | 2026-08-10 | The address system (ADR 0019): flag-never-suffix for companies (curated pins replace the QID suffix), non-ASCII slugs flag for curated romanization, generation mints strip nameplates and refuse drift-class slugs, the EPA leaf counter dies (collisions flag), stale slug-pair registry keys abort the consuming pass. |
| 24 | 2026-09-17 | The LLM read as a source (ADR 0017, amended 2026-09-17): the read pass mints, dates and places from verified quotes with a review flag per leaf; the placement pass defers to a stated placement and flags what rests on a read's span; `PLACEMENT_CORRECTIONS`. |
