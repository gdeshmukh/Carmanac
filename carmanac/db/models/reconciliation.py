...
# The addressable entity kinds (ADR 0019). Order is cosmetic; membership is
# the closed set the CHECK enforces. A future kind (platforms, trim lines)
# joins by migration: one kind string, one arc column - shape, never meaning.
ADDRESS_KINDS = (
    "company",
    "model",
    "model_line",
...
# Kinds whose addresses are scoped by owning company - the mirror of the live
# uniqueness constraints in hierarchy.py (uq_models_company_id_slug and
# siblings). Everything else is globally scoped.
COMPANY_SCOPED_KINDS = ("model", "model_line", "generation")

# kind -> the arc column that must carry its target. Kept as data because the
# migration's triggers and the CHECK below are both generated from it.
ALIAS_ARC_BY_KIND = {
    "company": "company_id",
    "model": "model_id",
...
ALIAS_REASONS = ("rename", "merge")


class SlugAlias(Base):
    """One retired address (ADR 0019): the slug an entity used to wear, and
    the row that answers for it now.

    Reconciler bookkeeping, not a fact table - no source ever asserted an
    alias, so there are no provenance columns (ADR 0002 satisfied by
    classification). Within the bookkeeping family it is a third species:
    durable adopted state, like raw records - a re-run cannot reproduce it,
    so nothing may reset it, and a from-scratch environment legitimately
    starts empty (it has served nothing).

    Rows are written by the database triggers installed alongside this table
    (rename path) and by gated merge scripts (merge path) - never by passes.
    Append-only, with exactly the two exceptions ADR 0019 sanctions: the
    trigger discharges a self-targeting alias when its entity re-acquires the
    address, and the gated script that created an alias for a never-served
    address may prune it while the pre-publication window is open.

    `entity_kind` + `scope_company_id` are the retired ADDRESS's kind and
    scope, frozen at retirement; the exclusive arc points at wherever the
    entity lives now. FKs deliberately block deletion (default NO ACTION):
    a row that history points at cannot go away until a script re-points
    the history.
    """
    ...
    scope_company_id: Mapped[int | None] = mapped_column(
        ForeignKey("companies.id", name="fk_slug_aliases_scope_company_id_companies")
    )
    slug: Mapped[str] = mapped_column(Text, nullable=False)

    # --- exclusive arc: exactly one set, matching entity_kind ---
    company_id: Mapped[int | None] = mapped_column(ForeignKey("companies.id"))
    model_id: Mapped[int | None] = mapped_column(ForeignKey("models.id"))
    model_line_id: Mapped[int | None] = mapped_column(ForeignKey("model_lines.id"))
    ...
    __table_args__ = (
        CheckConstraint(
            "entity_kind IN ({})".format(", ".join(f"'{k}'" for k in ADDRESS_KINDS)),
            name="entity_kind_valid",
        ),
        CheckConstraint(
            "reason IN ({})".format(", ".join(f"'{r}'" for r in ALIAS_REASONS)),
            name="reason_valid",
        ),
        # Scope column present exactly for the company-scoped kinds.
        CheckConstraint(
            "(entity_kind IN ({scoped})) = (scope_company_id IS NOT NULL)".format(
                scoped=", ".join(f"'{k}'" for k in COMPANY_SCOPED_KINDS)
            ),
            name="scope_matches_kind",
        ),
        CheckConstraint(
            "num_nonnulls({}) = 1".format(", ".join(ALIAS_ARC_BY_KIND.values())),
            name="exactly_one_target",
        ),
        # The arc column must be the kind's own. Cross-kind forwarding
        # (re-kinding a demoted generation into a trim line) is a future
        # ADR's migration, deliberately not a latent affordance here.
        CheckConstraint(
            "CASE entity_kind {} END".format(
                " ".join(
                    f"WHEN '{kind}' THEN {col} IS NOT NULL"
                    for kind, col in ALIAS_ARC_BY_KIND.items()
                )
            ),
            name="target_matches_kind",
        ),
        # One promise per address. NULLS NOT DISTINCT so two global-kind
        # aliases for the same slug collide despite their NULL scope.
        UniqueConstraint(
            "entity_kind",
            "scope_company_id",
            "slug",
            name="uq_slug_aliases_address",
            postgresql_nulls_not_distinct=True,
        ),
        Index("idx_slug_aliases_scope_company_id", "scope_company_id"),
        # Arc columns are sparse; partial indexes keep "index every FK" cheap.
        *(
            Index(f"idx_slug_aliases_{c}", c, postgresql_where=text(f"{c} IS NOT NULL"))
            for c in ALIAS_ARC_BY_KIND.values()
        ),
    )
