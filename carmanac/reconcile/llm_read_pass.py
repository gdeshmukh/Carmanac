...
flag. What a newer read no longer states withdraws: the placement
...
class LLMReadStats:
    ...
    def summary(self) -> str:
        return (
            ...
            f"dropped={self.dropped} | generations: matched={self.generations_matched} "
...
class LLMReadPass:
    ...
    def _process(self, record: RawRecord) -> None:
        ...
        for read in verified.generations:
            ...
            if generation is None:
                continue
            keys_now.add(f"read:{qid}#{slugify(read.name)}")
        ...
        prefix = f"read:{qid}#"
        for key, generation_id in self.read_keys.items():
            if key.startswith(prefix) and key not in keys_now:
    ...
    def _flag(self, configuration: Configuration, detail: dict, record: RawRecord) -> None:
        ...
        self.open_flags[configuration.id] = flag
